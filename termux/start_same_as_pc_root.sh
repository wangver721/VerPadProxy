...
# 3) 启动 —— 用 setsid 让 mitmdump 进入新会话，并显式 </dev/null 断开 stdin/stdout/stderr，
#    否则 adb shell 启动它时会一直等管道关闭。
setsid nohup /data/data/com.termux/files/usr/bin/mitmdump \
  -s ./redirect_addon.py \
  --listen-host 0.0.0.0 \
  --listen-port 2345 \
  </dev/null >/sdcard/VerPadProxy/_mitmdump.log 2>&1 &
